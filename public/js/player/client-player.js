...
import { cookies, transferPlayback, play } from "./client-player-api.js";
import { addEventOnElement, msToTimeCode } from "../utils.js"
...
let /** {Array<HTMLElement>} | undifined*/ $lastActivePlayBtns = []
const uptadeCardPlayBtnState = (playerState) => {
    const {
        paused,
        context: { uri },
        track_window: {
...
const uptadePlayerBtnState = (playerState, $player) => {
...
const updateDocumentTitle = (playerState) => {
    ...
    document.title = paused ? documentTitle : `${trackName} • ${artistNameStr} | Spotify`
}
...
const playerStateChange = (playerState) => {
    ...
    uptadeCardPlayBtnState(playerState);

    // update player control play btn ui state after state change
    $players.forEach($player => uptadePlayerBtnState(playerState, $player))
    ...
    // disable next and prev button if there is no track change
...
const togglePlay = async function (player) {
    ...
    if (playBtn === "play") {
        ...
        btnUri ? reqBody.context_uri = btnUri.replace(/"/g, '') : null;
        btnTrackUri ? reqBody.uris = [btnTrackUri.replace(/"/g, '')] : null;
...
window.onSpotifyWebPlaybackSDKReady = () => {
    ...
    player.addListener('ready', async ({ device_id }) => {
        ...
        addEventOnElement($playBtns, "click", function () {
    ...
    // call event when ant changes accur in player
